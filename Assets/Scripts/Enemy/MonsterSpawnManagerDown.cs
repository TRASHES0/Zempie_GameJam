...
public class MonsterSpawnManagerDown : MonoBehaviour
{
    [SerializeField]
    ...
    async void SpawnMonster(int spawn_num, int delay_time, Types type){
        if(type == Types.Red)
            ...
            for(int i = 0; i < spawn_num; ++i){
                Instantiate(_MonsterObj_Blue, transform.position, transform.rotation).GetComponent<EnemyMovement>()._targetpos = new Vector2(-10f, -3f);
                await UniTask.Delay(delay_time);
            }
    }
